...
const Rendering = () => {
    ...
    const ProductList = ProductName.filter((item) => item.price > 500 && item.price < 1000 || item.price > 1400)
    // const ProductList = ProductName.filter((item) => item.category === 'laptops' && item.price < 1400)
            ...
            <ul className='flex flex-wrap justify-around p-12'>
                {
                    ProductList.map((item) => {
                        return (
                            <div key={item.id} className='h-52 w-52 border-2 border-black mb-2 text-center'>
                                <p>{item.title}</p>
                                <p>{item.price}</p>
                                <img className='h-24 mx-auto' src={item.images} alt="" />
                            </div>
                        )
                    })
                }
